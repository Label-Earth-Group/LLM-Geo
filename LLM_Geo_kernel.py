...
class Solution():
    ...
    def get_LLM_responses_for_operations(self, review=True):
        ...
        for idx, operation in enumerate(self.operations):
            ...
            if review:
                for content in self.yield_ask_LLM_to_review_operation_code(operation):
                    yield content
            # time.sleep(25)
        return self.operations
    ...
    def yield_LLM_responses_for_operations(self, review=True):
        ...
        for idx, operation in enumerate(self.operations):
            node_name = operation['node_name']
            yield f"{idx + 1} / {len(self.operations)}, LLM is generating code for operation node: {operation['node_name']} \n"
            prompt = self.get_prompt_for_an_opearation(operation)
    ...
    def prompt_for_assembly_program(self):
        ...
        assembly_requirement = '\n'.join(
            [f"{idx + 1}. {line}" for idx, line in enumerate(constants.assembly_requirement(self.save_dir))])
    ...
    def yield_LLM_assembly_response(self, review=True):
        ...
        if review:
            for content in self.yield_ask_LLM_to_review_assembly_code():
                yield content
            self.save_solution()

    def save_solution(self):
        #         , graph=True
        new_name = os.path.join(self.save_dir, f"solution.pkl")
    ...
    @staticmethod
    def replace_print_with_yield(code):
        """replace "print" clause with yield"""
        pattern = r'{print\(\'(.*?)\'\)}'
        return re.sub(pattern, r"yield '\1\\n'\n", code)

    def yield_execute_complete_program(self, try_cnt: int = 10) -> str:
        ...
        is_success = False
        for count in range(try_cnt):
            print(f"\n\n-------------- Running code (trial # {count + 1}/{try_cnt}) --------------")
            yield f"\n\n-------------- Running code (trial # {count + 1}/{try_cnt}) --------------\n\n"
            try:
                compiled_code = compile(self.exec_code, 'Complete program', 'exec')
                for content in exec(compiled_code, globals()):  # #pass only globals() not locals()
                    print(content)
                    yield content
                is_success = True
                # !!!!    all variables in code will become global variables! May cause huge issues!
            ...
            #
                ...
                # cl, exc, tb = sys.exc_info()
                # print("An error occurred: ", traceback.extract_tb(tb))
                error_info_str, debug_prompt = self.get_error_info_and_debug_prompt(exception=err, code=self.exec_code)
                print(f"Error_info_str: \n{error_info_str} \n")
                yield f"Error_info_str: \n{error_info_str} \n"
                ...
                for content in self.yield_LLM_reply(prompt=debug_prompt,
                                                      ...
                                                      stream=True,
                                                      retry_cnt=5,
                                                      container_for_response=response
                                                      ):
                    yield content
        ...
        if not is_success:
            yield f"Failed to execute and debug the code within {try_cnt} times. \n\n"
    ...
    def get_debug_prompt(self, exception, code):
        ...
        debug_requirement_str = '\n'.join(
            [f"{idx + 1}. {line}" for idx, line in enumerate(constants.debug_requirement(self.save_dir))])
    ...
    def get_error_info_and_debug_prompt(self, exception, code):
        etype, exc, tb = sys.exc_info()
        exttb = traceback.extract_tb(tb)  # Do not quite understand this part.
        # https://stackoverflow.com/questions/39625465/how-do-i-retain-source-lines-in-tracebacks-when-running-dynamically-compiled-cod/39626362#39626362
        ...
        return error_info_str, debug_prompt
    ...
    def yield_ask_LLM_to_review_operation_code(self, operation):
        code = operation['operation_code']
        operation_prompt = operation['operation_prompt']
        review_requirement_str = '\n'.join(
            [f"{idx + 1}. {line}" for idx, line in enumerate(constants.operation_review_requirement)])
        ...
        reply_content = helper.extract_content_from_LLM_reply(response)
        if (reply_content == "PASS") or (new_code == ""):  # if no modification.
            yield "Code review passed, no revision.\n\n"
            new_code = code
        operation['code'] = new_code
    ...
    def yield_ask_LLM_to_review_assembly_code(self):
        code = self.code_for_assembly
        assembly_prompt = self.assembly_prompt
        review_requirement_str = '\n'.join(
            [f"{idx + 1}. {line}" for idx, line in enumerate(constants.assembly_review_requirement)])
